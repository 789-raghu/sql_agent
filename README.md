# Self-Hosted ClickHouse Natural Language SQL Agent

A fully self-hosted Natural Language → ClickHouse SQL agent powered by local CPU LLM inference (`Qwen2.5-Coder-7B-Instruct-GGUF`), `llama.cpp` (`llama-server`), `LangGraph`, `FastAPI`, and robust AST SQL security validation.
...
- **ClickHouse Native**: Queries ClickHouse data lake (`epdatalake` schema, `ht_amr_data`, etc.) via HTTP interface (`10.150.19.153:8123`) using Basic Auth (`epdatalake:glummonkey60`).
- **8-Layer Security Boundary**:
  1. Input validation & category classification
  2. AST SQL parsing (`sqlglot`) with ClickHouse dialect & single-statement enforcement
  ...
  6. EXPLAIN evaluation and query cost guardrails
  7. Read-only ClickHouse HTTP execution params (`readonly=1`)
  8. Configurable statement execution timeout (`SQL_STATEMENT_TIMEOUT_MS=5000`).
...
- `GET /health/database` - ClickHouse HTTP interface connectivity (`10.150.19.153:8123`)
